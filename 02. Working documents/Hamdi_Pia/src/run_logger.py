"""
Run logger — writes a compact per-session log to Hamdi_Pia/runs/.
...
def log_evals_bundle(run_id: str, bundle: Any) -> None:
    """
    Write human-readable eval summaries to the run log.
    `bundle` is an EvalsBundle or dict with the same shape.
    """
    if bundle is None:
        return
    ...
    log_event(run_id, "-" * 40)
    log_event(run_id, "[EVALS] Bundle updated")
    log_event(
        run_id,
        f"[EVALS] Source: {data.get('proposal_source', '?')}  "
        f"upload={data.get('proposal_upload_name') or '(none)'}",
    )

    client = data.get("client_eval")
    if client:
        if hasattr(client, "model_dump"):
            client = client.model_dump()
        log_event(
            run_id,
            f"[EVALS] Client review — verdict={client.get('overall_verdict')} "
            f"score={client.get('score')}/100",
        )
        takeaway = (client.get("headline_takeaway") or "")[:200]
        if takeaway:
            log_event(run_id, f"[EVALS]   takeaway: {takeaway}")

    loss = data.get("loss_eval")
    if loss:
        if hasattr(loss, "model_dump"):
            loss = loss.model_dump()
        log_event(
            run_id,
            f"[EVALS] Loss-risk — likelihood={loss.get('loss_likelihood')} "
            f"risk_score={loss.get('loss_risk_score')}/100 "
            f"verdict={loss.get('save_or_kill_verdict')}",
        )
        punch = (loss.get("punchline") or "")[:200]
        if punch:
            log_event(run_id, f"[EVALS]   punchline: {punch}")

    consistency = data.get("consistency_eval")
    if consistency:
        log_event(
            run_id,
            f"[EVALS] Consistency QC — score={consistency.get('overall_score')} "
            f"passed={consistency.get('passed')} "
            f"critical={consistency.get('summary', {}).get('critical', 0)} "
            f"major={consistency.get('summary', {}).get('major', 0)} "
            f"minor={consistency.get('summary', {}).get('minor', 0)}",
        )
        breakdown = consistency.get("score_breakdown") or {}
        if breakdown:
            parts = [
                f"{k}={round(v, 1)}"
            ...
            if parts:
                log_event(run_id, f"[EVALS]   deductions by check: {', '.join(parts[:8])}")
        for issue in (consistency.get("issues") or [])[:5]:
            if not isinstance(issue, dict):
                continue
            log_event(
                run_id,
                f"[EVALS]   issue [{issue.get('severity')} / {issue.get('type')}]: "
                f"{(issue.get('description') or '')[:120]}",
            )

    log_event(run_id, "-" * 40)
    append_evals_json_to_log(run_id, data)


def append_evals_json_to_log(run_id: str, bundle_dict: dict) -> None:
    """Append machine-readable evals JSON block to the run log file."""
    _, log_path = get_run_logger(run_id)
    try:
        export = {
            "evals_export": {
                "run_id": bundle_dict.get("run_id"),
                "updated_at": bundle_dict.get("updated_at"),
                "proposal_source": bundle_dict.get("proposal_source"),
                "client_eval": bundle_dict.get("client_eval"),
                "loss_eval": bundle_dict.get("loss_eval"),
                "consistency_eval": bundle_dict.get("consistency_eval"),
            }
        }
        block = json.dumps(export, indent=2, ensure_ascii=False, default=str)
        with log_path.open("a", encoding="utf-8") as fh:
            fh.write("\n--- EVALS_JSON_EXPORT ---\n")
            fh.write(block)
            fh.write("\n--- END_EVALS_JSON_EXPORT ---\n")
    except OSError:
        pass
