...
def run_agent(
    ...
    model : str | None
        Optional override for the OpenAI model identifier. When None, falls
        back to the module-level `MODEL` (sourced from `OPENAI_MODEL` env
        var). Used by the per-step model selectors in the Streamlit UI to
        pick faster vs. higher-capability models per agent.
    """
