...
Return JSON `{ "html_body": "<section …>…</section>", "notes": "…" }`. Nothing else.

### Authoring contract — every slide

- Root element MUST be `<section class="slide" data-layout="…">`. Use `data-layout="Title"` only for the cover slide (slide_index == 0). Use `data-layout="2/3"` when the slide naturally splits into a primary 2/3 column with a side panel. Otherwise use `data-layout="Default"`.
...
- Numbered indicators are circular `<div>`s with `border-radius: 50%` (rectangular shapes never get rounded corners).
- Text colour: black (`#000000`) only. Highlights use `var(--electric-blue-900)` background or `var(--cyan-lightest)` accent. No coloured body text.
...
- Confidence flags: when the input slide's `confidence` is `partial` or `placeholder`, append a small inline note `<p style="font-size:18px; font-style:italic;">Placeholder — pending partner confirmation</p>` inside `.content` so the BA sees the gap.
...
- Output ONLY the JSON `{ "html_body": "...", "notes": "..." }`. No prose around it.
...
|-------|------|-------------|
| html_body | string | The full `<section class="slide" …>…</section>` block, ready to be wrapped in the standard boilerplate. |
| notes | string | Short note to the BA — anything they should know about this slide (e.g. "uses placeholder text for fees", "Why McKinsey row 4 is partner-confirm pending"). Empty string when there is nothing to say. |

```json
{
  "html_body": "<section class=\"slide\" data-layout=\"Default\">\n  <div class=\"title\">\n    <div data-pptx=\"placeholder\" data-ph-idx=\"0\"\n         style=\"position:absolute; left:1.2px; top:28.7px; width:1745.3px; height:115.2px\">\n      <p>McKinsey is the right partner because we combine sector pattern recognition, integrated capability, and senior facilitation</p>\n    </div>\n  </div>\n  <div class=\"content\">\n    <div style=\"display:grid; grid-template-columns:repeat(3, 1fr); gap:32px; height:100%; align-content:start;\">\n      <div style=\"display:flex; gap:16px; align-items:flex-start;\">\n        <div style=\"width:48px; height:48px; border-radius:50%; background:#061F79; display:flex; align-items:center; justify-content:center; flex-shrink:0;\">\n          <span style=\"font-size:24px; color:#FFFFFF; font-weight:bold;\">1</span>\n        </div>\n        <p style=\"font-size:24px;\">Six of the ten largest European utilities have run comparable decarbonisation strategies with us</p>\n      </div>\n      <div style=\"display:flex; gap:16px; align-items:flex-start;\">\n        <div style=\"width:48px; height:48px; border-radius:50%; background:#061F79; display:flex; align-items:center; justify-content:center; flex-shrink:0;\">\n          <span style=\"font-size:24px; color:#FFFFFF; font-weight:bold;\">2</span>\n        </div>\n        <p style=\"font-size:24px;\">Integrated benches across strategy, corporate finance, and operations let us solve the stranded-asset, funding, and capability question in one engagement</p>\n      </div>\n      <div style=\"display:flex; gap:16px; align-items:flex-start;\">\n        <div style=\"width:48px; height:48px; border-radius:50%; background:#061F79; display:flex; align-items:center; justify-content:center; flex-shrink:0;\">\n          <span style=\"font-size:24px; color:#FFFFFF; font-weight:bold;\">3</span>\n        </div>\n        <p style=\"font-size:24px;\">Senior partner facilitation through the board-readiness moments other firms cannot match</p>\n      </div>\n    </div>\n  </div>\n  <div class=\"source\">\n    <p>Source: Partner answers; European Energy Practice credentials</p>\n  </div>\n</section>",
